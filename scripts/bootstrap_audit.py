...
def module_skeleton(source: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        if isinstance(source.get("consumers"), list)
        else [],
        "evidence_ids": [],
        "status": "unreviewed",
    }
...
def routing_snapshot(profile: dict[str, Any]) -> dict[str, Any]:
    if not profile:
        return {
            "status": "unavailable",
            "applicable_profiles": [],
            ...
            "recommended_readiness_gates": [],
        }
    return {
        "status": "collector_output_requires_review",
        "applicable_profiles": profile.get("applicable_profiles", []),
        "recommended_perspectives": profile.get("recommended_perspectives", []),
        "recommended_references": profile.get("recommended_references", []),
        "recommended_readiness_gates": profile.get("recommended_readiness_gates", []),
...
def bootstrap(discovery: dict[str, Any], project_name: str | None = None) -> dict[str, Any]:
    ...
    profile = collector_result(discovery, "project_profile")
    ...
    return {
        ...
        "project_profile": routing_snapshot(profile),
        "scores": {
            "confirmed_completion": None,
            ...
            "ml_production_readiness": None,
            "clinical_readiness": None,
            "hardware_readiness": None,
            "system_integration_readiness": None,
            "confidence": None,
        },
        ...
        "limitations": [
            ...
            "Project-profile routing hints must be confirmed from project intent and source context before domain conclusions are drawn.",
        ],
        "history": {},
    }
        ...
        json.dumps(
            {
                ...
                "applicable_profiles": result["project_profile"].get("applicable_profiles", []),
            },
            indent=2,
        )
    )
