...
def test_architecture_inference_detects_internal_edges_and_cycle(tmp_path: Path) -> None:
    ...
    write(tmp_path / 'core' / 'service.py', 'from data.repo import load\n\ndef run():\n    return load()\n')
    write(tmp_path / 'data' / '__init__.py', '')
    write(tmp_path / 'data' / 'repo.py', 'from core.service import run\n\ndef load():\n    return 1\n')
...
def test_readiness_gate_reports_bounds_and_coverage_without_inventing_score() -> None:
    gate = {
        ...
        'criteria': [
            {'id': 'workflow', 'status': 'pass', 'weight': 3, 'confidence': 0.9, 'critical': True},
    ...
    discovery = {
        ...
        'collectors': {
            ...
            'architecture': {
                ...
                'result': {
                    'nodes': [
                        {'id': 'api', 'name': 'api', 'path': 'api', 'dependencies': ['core'], 'consumers': []},
                        {'id': 'core', 'name': 'core', 'path': 'core', 'dependencies': [], 'consumers': ['api']},
                    ],
                    'edges': [
                        {
                            'from': 'api', 'to': 'core', 'kind': 'static_import', 'confidence': 0.95,
                            'occurrences': 2,
                            'evidence': [{'path': 'api/routes.py', 'line': 3, 'declaration': 'core.service'}],
                        }
                    ],
                    'cycles': [],
...
def test_snapshot_comparison_tracks_debt_bottlenecks_and_methodology_change() -> None:
    old = {
        ...
        'modules': [{'id': 'core', 'completion': 70, 'quality': 80, 'classification': 'critical_path'}],
        'risks': [{'id': 'r1'}],
        'technical_debt': [{'id': 'd1'}],
        'bottlenecks': [{'id': 'b1'}],
        'readiness_gates': {
            'mvp': {'evaluation': {'score': 65, 'coverage': 80, 'confidence': 75, 'state': 'nearly_ready'}}
        },
    }
    new = {
        'metadata': {'auditor_version': '0.3.0'},
        'scores': {'estimated_completion': 75},
        'modules': [{'id': 'core', 'completion': 75, 'quality': 78, 'classification': 'critical_path'}],
        'risks': [{'id': 'r2'}],
        'technical_debt': [{'id': 'd2'}],
        'bottlenecks': [],
        'readiness_gates': {
            'mvp': {'evaluation': {'score': 74, 'coverage': 100, 'confidence': 90, 'state': 'ready'}}
        },
    }
...
def test_dashboard_exposes_readiness_and_trajectory_views() -> None:
    audit = {
        'metadata': {'generated_at': '2026-01-01T00:00:00+00:00', 'auditor_version': '0.3.0'},
        'project': {'name': 'Demo'},
